...
export default {
  ...
  methods: {
    loginclick: async function () {
      ...
      fetch("http://127.0.0.1:5000/api/vueadmin_login", {
        method: "POST",
        headers: {
        ...
        .then((response) => {
          ...
          if (response.status === 200) {
            ...
            // Login successful, save the token to localStorage or a cookie
            localStorage.setItem("access_token", data.access_token);
            if (data.isadmin){
              this.$root.$emit("loginStatusChanged", true);
              this.$router.push("/vueadmin_dashboard");
            }else {
        ...
        // Decode the payload (second part of the token)
              ...
              this.$router.push(`/vueuserdashboard/${user_id}`);
